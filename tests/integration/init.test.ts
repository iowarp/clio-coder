...
import { readCodewiki } from "../../src/domains/context/codewiki/indexer.js";
...
describe("clio init", () => {
	it("writes CLIO.md, .clio/state.json, and .gitignore for a fixture repo", async () => {
		...
		try {
			...
			strictEqual(result.summary.codewikiEntries, 1);
			...
			strictEqual(codewiki?.entries.length, 1);
		} finally {
	...
	it("prints compact init output instead of dumping git status", async () => {
		...
		try {
			...
			ok(rendered.includes("codewiki rebuilt 1 entry"), rendered);
