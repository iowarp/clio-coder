...
describe("createCoalescingChatRenderer", () => {
	function setup(coalesceMs = 5): {
		...
		const renderer = createCoalescingChatRenderer({
			...
			clearTimer: (id) => {
				if (pendingTimer === id) pendingTimer = null;
			},
		});
		return {
			renderer,
	...
	it("coalesces 100 text_delta events into one requestRender call", () => {
		const { renderer, applied, count, fireTimer } = setup(5);
		...
		// One timer render covers all 100 deltas.
		fireTimer();
		strictEqual(count(), 1);
	});

	it("message_end after deltas renders synchronously and drops the pending timer", () => {
		const { renderer, count, fireTimer } = setup(5);
	...
	it("agent_end renders synchronously even with no pending deltas", () => {
		const { renderer, count } = setup(5);
		renderer.applyEvent({ type: "agent_end", messages: [] });
		strictEqual(count(), 1);
	});

	it("flush() drains a pending coalesce timer and renders once", () => {
		const { renderer, count, fireTimer } = setup(50);
