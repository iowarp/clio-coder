...
describe("commit attribution message policy", () => {
	...
	it("adds only a full, directly relevant, integrity-valid receipt-v15 digest", () => {
		const digest = "a".repeat(64);
		const valid: CommitAttributionEvidence = {
			materiallyAssisted: true,
			receipt: { version: 15, algorithm: "sha256", digest, integrityValid: true, directlyRelevant: true },
		};
		match(attributeCommitMessage(subject, valid), new RegExp(`Clio-Evidence: receipt-v15/sha256:${digest}`));
		strictEqual(
			attributeCommitMessage(subject, {
				...valid,
				receipt: {
...
describe("managed prepare-commit-msg attribution", () => {
	...
	it("ignores testing, review, and evidence claims exported by the child shell", () => {
		...
		const forged = {
			...
			CLIO_CODER_COMMIT_EVIDENCE: `receipt-v15/sha256:${"b".repeat(64)}`,
		};
