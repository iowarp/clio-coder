...
import { runNodeScript } from "../harness/spawn.js";
...
describe("contracts/live home", { concurrency: false }, () => {
	...
	it("sends an eval item's state dir inside the home rather than to the machine tmpdir", async () => {
		...
		const outsideBefore = readdirSync(scratchRoot).filter((name) => name.startsWith("clio-eval-state-"));
		try {
			...
			const outsideAfter = readdirSync(scratchRoot).filter((name) => name.startsWith("clio-eval-state-"));
