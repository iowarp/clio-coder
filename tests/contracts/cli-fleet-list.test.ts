/**
 * What `clio-coder fleet list` says about the shipped fleets in a repo that has not
 ...
 * `.clio-coder/fleets/commands.yaml` and no way to produce it. Nothing about those
...
describe("contracts/cli-fleet-list", () => {
	...
	function writeRegistry(yaml: string): void {
		mkdirSync(join(repo, ".clio-coder", "fleets"), { recursive: true });
		writeFileSync(join(repo, ".clio-coder", "fleets", "commands.yaml"), yaml);
	}
	...
	it("calls a builtin that only needs the repo's command registry `setup`, with the ids and the remedy", async () => {
		...
		match(
			result.stdout,
			/^build-test {2}builtin {2}setup {4}needs \.clio-coder\/fleets\/commands\.yaml declaring test$/m,
		);
		match(
			result.stdout,
			/^sdlc {2}builtin {2}setup {4}needs \.clio-coder\/fleets\/commands\.yaml declaring commit, test$/m,
		);
		match(result.stdout, /declare each id there under `commands:` with an `argv` list/);
		match(result.stdout, /`clio-coder docs fleet_dispatch` has the schema/);
	...
	it("names the same remedy when the fleet is run rather than listed", async () => {
		...
		match(result.stderr, /code steps require a command registry at \.clio-coder\/fleets\/commands\.yaml declaring test/);
		match(result.stderr, /`clio-coder docs fleet_dispatch` has the schema/);
	});
