...
- Added bounded parent-to-worker dispatch briefings as separate untrusted dynamic data with registry-owned approval pinning and byte/hash receipt provenance; added a nullable four-code structured terminal `outcomeCode` retry policy sourced only from trusted Clio runtime seams; and advanced new receipt writes to v5 while preserving genuine historical v4 verification and rejecting v4 artifacts that carry v5-only fields.
