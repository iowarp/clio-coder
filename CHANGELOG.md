...
- Tool-result offload scratch files older than fourteen days are now swept at boot (#196). A session resumed after that retention window can still show a full-output path from its ledger even though the referenced scratch file no longer exists.
...
- A turn is attributed to the model that served it under LM Link peer routing (#185). The response's `model` field is recorded as `responseModel` when it differs from the request; the footer's last-turn line adds `served <id>`, `/cost` and `clio-coder usage report` attribute tokens to the served id with the requested ids beside it (`requestedModel`/`servedModel` on every ledger call, a `requested` column and `requestedModels`/`servedCalls` on the `model-usage` fact), and the LM Link peer warning is emitted once per distinct fact instead of once per turn.
