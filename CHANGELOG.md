...
- Fixed an undiscovered context window being reported as a capability. A router answering `/props` with `n_ctx: 0` publishes its real window in the `/v1/models` row, which was parsed and never read, so a placeholder rendered exactly like a discovered value. There is now one exact-id selector for the probe layer, and `TargetStatus.contextWindowProvenance` names which layer answered. A window nothing declared renders as an unverified runtime default, and `configure` warns as it writes one.
