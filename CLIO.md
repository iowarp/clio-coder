...
- `clio context wiki --depth auto|simple|medium|detailed` selects the strategy, and `--target`, `--model`, and `--thinking` pin an exact route. `src/domains/context/wiki/plan.ts` is the one planner: `auto` classifies from indexed source files and lines, and `WIKI_DEPTH_STRATEGY` is the single table where coverage-anchor hints, target page range, and target substantive-size floor are chosen.
- Wiki generation reuses the shipped `documenter` agent with a task-specific prompt. Wiki generation does not fan out source areas to Scout researchers: independent summaries duplicated exploration, lost cross-domain relationships at the handoff, competed for dispatch admission, and left the final author responsible for validating compressed claims it did not observe.
- Init and update are normally one documenter pass. A second pass is allowed only when the writer exhausts its tool budget or deterministic validation finds a real defect in the staged artifact.
- Artifact validation before promotion is the authority. Temporary `_plan.md`/`plan.md` notes are removed before validation. `validateWikiLayoutInDir` rejects a wiki that is missing `quickstart.md`, contains an empty page, cites a concrete source path that does not exist (TypeScript-authored `.js` module specifiers resolve to their `.ts`/`.tsx` source; globs are not concrete citations), or contains an internal wiki link to a page that does not exist. Page range and byte floor are guidance for the documenter prompt, not hard validation rules.
- A recoverable writer failure hands what it staged to validation rather than discarding it, and a staged wiki that passes every deterministic check is promoted with the outcome named in progress. The writer stopping on its budget is a fact about the writer, not about the artifact, and deleting a correct wiki over it lost work the run had already done. Any other nonzero outcome fails the run before promotion, and a staged tree that fails validation fails the run carrying both the writer's outcome and the validation problems. Transactional promotion, locking, metadata, no-op behavior, and staging write containment are unchanged.
- The wiki dispatch is not offered `git`. `buildWikiPrompt` already embeds `git status` and `git log` verbatim, so `op=status` and `op=log` re-fetch text the model was handed, and the staging dir sits under the gitignored `.clio/`, so `op=diff` is blind to the pages the run is writing. All three ops were non-productive and took about 40% of the documenter's orientation. `JobSpec.denyTools` is the subtraction that removes it: it only ever shrinks the surface admission already resolved, so any origin may set it, a name matching no Clio tool is inert, and removing a tool the recipe *requires* fails admission rather than running the agent without it. The `init` and `update` fragments no longer tell the writer to reach for it, because a prompt that names a tool the run does not have spends budget teaching the model that.
- A page with no H1 fails validation. `titleFromPage` falls back to the file name, so such a page lands in `meta.json` titled `providers.md`. Validation and the title reader share one `wikiPageHeading`, so a page that promotes is exactly a page whose title the reader could find, and anything checkable is checked here rather than asked for in a prompt sentence the writer may skip.
- `WikiMetaGeneration` records only what was observed and chosen: requested depth, resolved depth, source files, source lines, and the researcher count retained for metadata compatibility (zero for the single-owner strategy). Page, size, and pass bounds are policy derivable from `depth`, so storing them would duplicate a constant the reader can look up. The block is optional, and metadata written before it stays valid.
...
- A skill root offers only what its scope contains. A discovered skill must resolve inside the workspace for a project root and inside the home directory for a user root, the same boundary `assertWriteBoundaryInsideRoot` draws. `.clio/skills` is trusted unconditionally, so before this a cloned repository could ship `.clio/skills/x -> /home/victim/.ssh` and have it load as an ordinary model-visible project skill whose body became trusted instructions and whose `include_tree` enumerated the target. The escape was invisible in every path the operator is shown, because `filePath` and `baseDir` both keep the symlink's own repository-local location. A monorepo may still share one skills tree across checkouts, because the boundary is the workspace rather than the skills directory. Extension roots and explicit skill paths carry no anchor: a package manager symlinks by design, and an explicit path names its own target.
- A SKILL.md is validated as a document before it becomes instructions. The decode is round-tripped because `readFileSync(path, "utf8")` never throws on binary and substitutes U+FFFD instead, so a binary file loaded as a document of replacement characters. Over 1 MiB it does not load at all, since `loadSkills` runs on every `context(scope="skills")` call and reads and hashes each file whole. Over `OBSERVE_SELF_CAPS.contextSkills` it still loads and says the model will not be shown the rest, because truncating is the activation path's job but an author has no other way to learn their workflow is delivered cut in half. The cap is mirrored rather than imported, because the tool substrate is not a domain's to depend on, and a contract test asserts the mirror.
- Referenced-file validation is deliberately absent, and this is a format limit rather than a missing check. A skill body names paths as bare code spans with nothing distinguishing a skill resource from a workspace path from a file the workflow is telling the model to create. Across Clio's own sixteen shipped skills, 8 of 41 such paths resolve under the skill, 16 under the workspace, and 17 nowhere at all and correctly so. An existence check would fail 33 correct references. Making this checkable needs a declared resource manifest in frontmatter.
- Drift has two authorities and they answer different questions. The pinned catalog manifest says what the audited entry's content is and wins wherever it has an entry. The skill's own `installed-hash` frontmatter says what this machine wrote, which is the only evidence for a skill installed straight from a URL and the only thing that can see content edited on disk after installation. Both compare on the provenance-stripped hash, so install-lifecycle stamps never read as drift while any content or registry-identity edit does. The activation warning names which authority spoke and what it expected. Nothing recorded means no verdict; drift evidence is opt-in and never a gate.
- An update passes the gate a fresh install passes. Upstream may have moved on to a SKILL.md that no longer loads, and writing it over a working skill tells the operator it updated while the skill silently stops existing. A refused update leaves the installed copy byte-identical. Stripping provenance removes the lines a key owns, not just the key line, so a value spanning lines cannot leave a continuation that YAML then reads as part of the preceding key; hashing and injection share one removal so the two cannot disagree.
- `clio skills eval <name>` resolves the copy an activation would load. Discovery's precedence winner comes first and the catalog answers only for a name no root installed, which is the skill author's case in this repository. The selected origin, path, and normalized hash are reported before any scenario runs, because a result nobody can attribute to a source is not evidence about a skill.
- Worker skill activations fold into the session ledger from both terminal dispatch channels. The run finalizer builds one payload and emits exactly one channel, so `foldDispatchSkillActivations` cannot double-record, and a run that failed after loading a skill is the case the provenance is most needed for.
- Skill narrowing is released by the next submit assigning a fresh policy, not by the `finally` restore, which fails nothing when removed and is defense in depth. Its two readers are both safe: one takes a callback invoked after the assignment, and one finalizes `ask_user`, which narrowing exempts. A worker needs no release, because one worker process serves one run and the policy dies with it.
...
- An optional authored field has three spellings of emptiness and all three mean absent: the key omitted, `null`, and a blank string. `ResultAuthorship` types `commitMessage` and `summary` as `string | null` and `resultContractAuthorship` normalizes a blank to null, so validation that refused them contradicted its own reader and ended finished runs: a wiki documenter that authored no commit sealed `{"commitMessage":"","summary":""}`, spent both repair rounds being told to make it non-empty, and failed with `result_contract_exhausted`. A non-string is still a shape error, because only emptiness is absence.
...
- A worker's budget is the recipe's, and `guardrails.workerToolCallCap` is the ceiling it may not exceed. Dispatch resolves `min(declared, cap)`, so the smaller of the two always wins and a cap tuned for one shape of agent silently rewrites every recipe's budget. The cap is sized to leave the recipe binding, because only the recipe knows how many calls its job takes: a writer producing a dozen pages spends a call per page before its first grounding read.
- The cap bounds calls that execute, never calls the harness refused. A reserve-window steering block and a synthesis-lockout denial ran nothing, and charging them killed runs for obeying the guard: a documenter told to stop exploring and write had every write refused, retried, and charged until the lifetime cap aborted it with nothing promoted. Attempts stay bounded independently by the identical-call detector, the per-turn block budget, and the per-round synthesis backstop, and a loop block still costs because that one is the runaway signal.
- A reserve window ends discovery, not delivery. `deliveryTools` are the mutation tools the agent was actually granted; inside the reserve they are admitted beside `read` and everything else bounces, and an agent with none keeps a read-only reserve and the request-level `require_tool(read)` lock. The native guard in `src/engine/loop-guard.ts` and the Claude SDK gate in `src/engine/claude/sdk-runtime.ts` apply the same rule, because a promise about the last calls of a run cannot depend on which runtime served it.
- The soft budget is the end of that same window and ends the same thing. A recipe's declared `toolCalls` closes discovery for the rest of the run; an agent holding delivery tools keeps delivering, bounded by `hardCap`, which sits above the declared budget for exactly this. Refusing delivery at the boundary named a writer's own product "exploration" and told it exploration was disabled: measured on the wiki documenter, ten of one pass's twelve write attempts were refused with `worker exploration budget reached` and the pass landed no successful write at all. Past the limit a discovery call is steered by `workerDeliveryOnlyDirective` and escalates to the same graceful lockout the reserve escalates to. An agent with no delivery tools has nothing left to do with a tool call, so its budget still arms the synthesis lockout exactly as before, and `onSoftLimitFinalCallAdmitted` still fires, because an agent that declared no synthesis phase ends where it said it did.
- A worker's loop-block budget scales with its own tool budget through `workerLoopBlockBudget`, floored at the interactive `INTERACTIVE_LOOP_BLOCK_BUDGET`. A worker sets no turn id, so every block it collects lands in one bucket that lives for the whole run, and measuring that run-long count against a per-turn budget of two meant the second verbatim spiral anywhere in a ninety-call editing pass locked the run to synthesis and the backstop then ended it. The identical-call detector still blocks each spiral where it happens; this is the separate judgment that the run as a whole is not converging, so it is a statement about run length. The bucket stays run-long deliberately: per-round buckets would let a model that spends one blocked call per round spin forever, now that refused calls are free.
- Refused attempts carry one run-level bound that is blind to correlation: a run may be refused at most as many calls as it may execute, floored at `WIDE_BATCH_DENIAL_FLOOR`, and past that it ends on the backstop the worker aborts on. Every other bound here is per model round, which is right for steering a model that answers with tool calls but assumes a round is a handful of them. A degenerate local model emits one response whose parsed calls number in the hundreds, all sharing a correlation id, so the per-round bounds see one denial while the batch drains. Charging refusals to the lifetime cap used to stop that by accident; this is the same bound stated directly. The orchestrator has no cap and no bound here, because it has an operator.
- A run that declares write roots is not offered the tools confinement refuses by name. `WRITE_ROOT_REFUSED_TOOLS` in `src/domains/safety/policy-engine.ts` is the one list, used both to refuse at the seam and to narrow the offered surface, so the two cannot drift. An offered tool whose every call is refused is budget the model spends learning it cannot use it, and the refusal reads to the model as a mistake worth retrying. A recipe that *requires* one of them under confinement fails admission rather than running without it.
- Refusals inside a reserve are themselves bounded per model round. A model that keeps calling discovery tools there reaches the same graceful synthesis lockout the spent soft budget ends in, rather than bouncing forever now that the refusals are free.
