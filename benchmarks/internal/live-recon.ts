...
await runDriver(USAGE, async () => {
	...
	const home = prepareLiveHome(args, { prefix: "clio-live-recon-", autonomy: "full-auto" });
	...
	try {
		...
		process.stderr.write(`live recon: FAIL: ${error instanceof Error ? error.message : String(error)}\n`);
	} finally {
		if (staleWikiDir) rmSync(staleWikiDir, { recursive: true, force: true });
		home.cleanup(passed);
	}
	return passed;
});
