...
those agents see only the run's environment, so pass their own tokens with --pass-env.
`;
...
await runDriver(USAGE, async () => {
	...
	// Cleanup is armed before anything runs inside the home: a thrown check, a
	// SIGINT, or a child that will not die all still remove the credentials.
	return withLiveHome(
		args,
		{
		...
		async (home) => {
			...
			try {
				...
				process.stderr.write(
					`\nlive smoke: FAIL: ${home.redact(error instanceof Error ? error.message : String(error))}\n`,
				);
				return false;
			}
		},
