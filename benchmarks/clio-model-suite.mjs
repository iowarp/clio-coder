...
function taskPrompt(runDir, combo) {
	return `You are being benchmarked directly. Do not use dispatch, monitor, steer, subagents, skill loading via context(scope="skills"), web_fetch, or any delegation. Use only local read/ls/write/edit style tools yourself. Hard limit: at most 4 inspection tool calls (you may read README.md if it exists and list the directory once to orient yourself), then you must call write to create ${runDir}/app.html. Build a standalone marketing landing page for "Clio Coder", a local-first AI coding CLI that runs open-weight models on your own hardware, manages model targets, and benchmarks them. Requirements for app.html: standalone HTML with embedded CSS, responsive layout, a hero, a model-management section, a benchmark section, docs links, accessibility labels, and no external network assets. Current benchmark config metadata: ${JSON.stringify(combo)}. Do not modify files outside ${runDir}.`;
}
