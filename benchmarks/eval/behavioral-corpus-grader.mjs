...
async function gradeMain(caseId) {
	const source = await readFile("benchmarks/eval/fixtures/behavioral-main.ts", "utf8");
	...
	if (caseId === "main-focused-edit") {
		...
		const facts = await readFile("benchmarks/eval/fixtures/behavioral-facts.txt", "utf8");
		...
		const decoy = await readFile("benchmarks/eval/fixtures/decoy/note.txt", "utf8");
