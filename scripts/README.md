...
| `node --import tsx scripts/decision-probe.ts <fixture> --profile <name>` | Score a decision site's wording against its labeled fixture under `tests/fixtures/decision-cases/` | **Calls the configured decision model**; binds the fixture's sites to the profile in memory only | Explicit operator run only |
