...
| `pnpm bench:boot -- --runs 5 [--cwd <dir>] [--user-plugins <dir>]` | `bench-boot.ts`: type into the built CLI from its first frame in a real PTY; report Stage 0, Stage 1 hydration, longest input block and keystroke echo latency | Isolated scratch home with a local stub target, optionally a copy of a plugin directory; cleaned up; no model calls | Explicit operator run only; observations, not thresholds |
