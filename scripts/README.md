...
| `node --import tsx scripts/decision-probe.ts <fixture> --profile <name>` | Score a decision site's wording against its labeled fixture under `tests/fixtures/decision-cases/`; `--cases` also writes each turn's values to stderr | **Calls the configured decision model**; binds the fixture's sites to the profile in memory only | Explicit operator run only |
| `pnpm bench:boot -- --runs 5 [--cwd <dir>] [--user-plugins <dir>] [--cli <entry>]` | `bench-boot.ts`: type into the built CLI from its first frame until the hydrated footer has settled, in a real PTY; report Stage 0, Stage 1 hydration, longest input block and keystroke echo latency. `--cli` points at another build's entry to compare two builds in one sitting | Isolated scratch home with a local stub target, optionally a copy of a plugin directory; cleaned up; no model calls | Explicit operator run only; observations, not thresholds |
