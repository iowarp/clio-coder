...
| 14 | Observability | Usage, cost, durable trace, run facts | **Present** | Visible tokens, historical usage, trace accounting to the phase, event and process shapes, and stored eval reports all cross through bounded fixed reads. |
...
`src/domains/eval`, `src/domains/evidence`, `src/domains/evolution`, `src/domains/gateway`, `src/domains/memory`, `src/domains/plugins`, `src/domains/user-tasks`. Note the audit's "Resources" row predates the split of `plugins` out of `resources`, which is what the `library` command rewrite rides on.
