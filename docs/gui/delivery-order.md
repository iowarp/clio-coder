...
5. **Historical evidence and economics — Usage partially implemented.** A fixed project-filtered JSONL adapter powers the 30-day Usage record and discards the upstream report's global or raw rows. Evidence has no JSON listing and trace runs are global and carry raw requests; **those boundaries remain visible in the GUI rather than being scraped around.** Never pass arbitrary CLI output to the browser.
   - *Status:* both named gaps are resolved (`evidence inventory --json`, `trace inspect --json`). The habit — make a boundary visible rather than scrape around it — is the part to keep.
