...
|---|---|---|---|
| `chat.maxOutputTokens` | `32768` | Global output-token budget requested on every chat turn (integer >= 0, `0` means per-model caps only); clamped to the model's max-output cap and remaining context window; applies next turn. | session override (`/settings` apply-this-session) > `.clio-coder/settings.local.yaml` > `.clio-coder/settings.yaml` > user settings.yaml > default |
...
| `fleet.concurrency` | `auto` | Maximum concurrently running local workers: `auto` derives it from host capacity, or an integer >= 1; applies at restart. |  |
...
| `fleet.default.thinkingLevel` | `False` | Saved default thinking level for workers (`off` through `max`); applies next session. | `run --thinking` > selected `fleet.profiles` route thinkingLevel > session routing (`/settings`) > `.clio-coder/settings.local.yaml` > `.clio-coder/settings.yaml` > user settings.yaml > default |
...
| `fleet.profiles.<key>.thinkingLevel` | `False` | Thinking level for workers routed through this profile (`off` through `max`); `off` when absent; applies next dispatch. |  |
...
| `interface.panes.layout` | `False` | Docks composed at interactive boot in guest mode: `off` opens nothing, `workers` opens the workers dock, `cockpit` opens workers and files; applies at restart. |  |
...
| `interface.smoothStreaming` | `False` | Presentation-only pacing of streamed assistant text and thinking: `off`, `auto` (TTY heuristics), `on`; applies immediately. | session override (`/settings` apply-this-session) > `.clio-coder/settings.local.yaml` > `.clio-coder/settings.yaml` > user settings.yaml > default |
...
| `CLIO_CODER_ALLOW_EXTERNAL_FULL_ACCESS` | `False` | Exactly `1` lets a `full-auto` dispatch to the claude-code or antigravity-code subprocess runtime pass their dangerous skip-permissions flag; any other value keeps the bypass closed. |  |
...
| `CLIO_CODER_BUS_TRACE` | `False` | Exactly `1` subscribes a tracer to the shutdown, session-end, and domain lifecycle bus channels and writes one `[clio-coder:bus]` line per event to stderr. |  |
...
| `CLIO_CODER_DEBUG_SHUTDOWN` | `False` | Exactly `1` prints timed `[clio-coder:shutdown]` phase lines and full stack traces of failing domain `stop()` hooks to stderr during shutdown. |  |
...
| `CLIO_CODER_FORCE_COMPACT` | `False` | Exactly `1` on the interactive process forces the pre-submit compaction to run on every turn while set, regardless of the context threshold. |  |
...
| `CLIO_CODER_HOOK_BUDGET_DEBUG` | `False` | Exactly `1` prints every post-warmup hook-budget overrun to stderr instead of only steady-state slowness. |  |
...
| `CLIO_CODER_INSTANT_SHELL` | `True` | Exactly `0` disables the Stage 0 instant interactive shell mounted before service hydration; any other value or unset keeps it on. |  |
...
| `CLIO_CODER_LEGACY_MASK` | `False` | Exactly `1` re-enables the destructive stale-observation mask stage before summary compaction; a one-release compatibility escape hatch. |  |
...
| `CLIO_CODER_MEMORY_TRACE` | `False` | File path for a JSONL trace of proactive task-memory step envelopes including up to 8000 chars of model text per step; off when unset or empty. |  |
...
| `CLIO_CODER_NO_NETWORK_TOOLS` | `False` | Exactly `1` removes the network (RETRIEVE) tools from every registry built in the process; `clio-coder skills-eval` sets it for hermetic arms and `--allow-network` deletes it. | `skills-eval --allow-network` clears it for child arms; otherwise env only |
...
| `CLIO_CODER_REDUCE_MOTION` | `False` | Exactly `1` makes smooth-streaming `auto` use the immediate coalescer; explicit `on` is unaffected. |  |
| `CLIO_CODER_RENDER_TRACE` | `False` | File path for the versioned JSONL render-pipeline trace (timing only, no conversation text), truncated on open; off when unset or empty. |  |
| `CLIO_CODER_REQUIRE_HOME_PREFIX` | `False` | Exactly `1` aborts startup when any resolved Clio directory lies outside `CLIO_CODER_HOME`; a test-harness guardrail that does nothing when `CLIO_CODER_HOME` is unset. |  |
...
| `CLIO_CODER_SCREEN_READER` | `False` | Exactly `1` makes smooth-streaming `auto` use the immediate coalescer so a screen reader gets the low-motion update behavior; explicit `on` is unaffected. |  |
...
| `CLIO_CODER_SYNTHESIS_LOCK` | `strip` | `tool-choice` keeps tool schemas and sends `tool_choice: none` on a synthesis-locked worker round for OpenAI-family runtimes; any other value strips the schemas. |  |
...
| `CLIO_CODER_TEST_STAGE1_FAIL` | `False` | Under `NODE_ENV=test` only, exactly `1` throws an injected Stage 1 hydration failure after the instant shell mounts. |  |
| `CLIO_CODER_TEST_UPGRADE_NO_NETWORK` | `False` | Any non-empty value makes `clio-coder upgrade` skip the npm install step so upgrade tests run offline. |  |
| `CLIO_CODER_TIMING` | `False` | Exactly `1` prints the startup timer report after the banner, but only on the bannered non-interactive boot (not the TUI, headless, or ACP paths). |  |
| `CLIO_CODER_TRACE_BOOT` | `False` | Exactly `1` writes a `[clio-coder:boot] +<ms> <phase>` line to stderr for every boot phase marker, including startup-timer marks. |  |
...
| `CLIO_CODER_WORKER_FAUX` | `False` | Exactly `1` registers the pi-ai faux provider and queues one deterministic assistant reply so a worker subprocess runs end to end without credentials (tests only). |  |
...
|---|---|---|
| `(recipe files)` | Project agent recipes (`<id>.md` with the recipe frontmatter schema); a project recipe must declare `audience: custom` and shadows a builtin of the same id. | project `.clio-coder/agents/` > user agents > builtin recipes, by id |
...
| `dispatch.roster` | policy | Configured workers.rosters name (council). |
