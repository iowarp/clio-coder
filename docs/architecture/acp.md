...
| `session_cwd_mismatch` | A session request names a canonical directory different from the bound workspace root. The message names that root. |
...
At most one `session/new`, `session/load`, or `session/resume` is hosted at a time. Another opener fails with `session_limit` until the hosted session closes. Closing releases the slot and resets the conversation so another session can open in the same process. The first bound workspace remains fixed for the lifetime of that process.

### Workspace pinning

`session/new`, `session/load`, and `session/resume` require a non-blank absolute `cwd` naming an existing directory. A relative path or an unusable directory fails with `-32602`. The first valid request binds the process before any workspace graph loads unless `--cwd` already bound it. A later request for another directory fails `session_cwd_mismatch`, with the bound root in the message. Clio never changes directories after the workspace graph loads.

Workspace authority remains the exact canonical bound directory, never the enclosing Git root. Workspace Git probes treat an ignored nested scratch directory as non-Git. An unignored monorepo subdirectory may truthfully inherit repository-level branch, upstream, and remote facts, but dirty status and recent commits are path-scoped to the exact workspace, and no parent path is returned.
...
`session/new`, `session/load`, and `session/resume` accept client supplied stdio `mcpServers`. Clio starts each declaration through the gateway's safe stdio MCP client, registers its tools as session scoped gateway capabilities named `mcp_acp_<name>__<tool>`, and routes calls through the normal tool registry and autonomy policy. These calls use the `unknown` action class, which asks at `default` autonomy and runs at `yolo`. Client declarations and catalogs are never written to settings or the MCP catalog cache. Closing the ACP session closes those clients and removes their registered tools. HTTP and SSE remain unsupported. Malformed or non-stdio declarations fail `invalid_params`. `additionalDirectories` may be empty; nonempty additional roots are unsupported. A loaded id must occur in the bound workspace's history and be durably closed. An unhosted record with `endedAt:null` fails `session_open`: Clio cannot prove whether it belongs to a live process or an unclean crash, and does not guess.
