...
Note: `src/engine/ai.ts` gained the `stream` re-export in slice 6 (`feat(diag): expose pi-ai stream through engine boundary`, `1e5997b`) so that the real-inference and vision diag scripts, the worker runtime, and the dispatch layer all reach the pi-ai streaming entrypoint through one engine module. Do not import `stream` (or any pi-ai symbol) directly outside `src/engine/`.
