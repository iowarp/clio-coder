...
- **Compact Mode (Two-Line Ambient Strip)**:
  - **Line 1 (Workload & Identity)**: Active phase (`accent` e.g. `Ready`) and worker counts (`agent` e.g. `· 2 active`) · the armed skill surface while one narrows the tools (`skill tdd` in `muted`, or `§ tdd` where the words do not fit, dropping whole names as `§ tdd…`; it outranks everything after it) · Target/model identity (`fitIdentityLabel`) · Selected model’s weekly quota (`weekly N% left`, when the local credential owner and model group are known) · Thinking level (`reason`) on the left; Context meter bar (8–14 cells), its percent and token occupancy (`used / contextWindow`) on the right. Nothing on the row is cut mid-number or cut to a stub. The phase and worker count take the room they need beside the skill badge, and a row too narrow for both drops the worker count before it cuts the phase. The identity shows only with at least 12 cells of room (`blade…q4_k_m`), and the thinking level takes the room it leaves. A meter whose counts do not fit in about half the row drops them and keeps the percent (`▰▱▱▱▱▱▱▱  12.9%`). The phase spinner steps once per 120 ms animation step, however often the footer refreshes, and a live elapsed counter shows whole seconds (`Writing · 3s`) and nothing under one second.
