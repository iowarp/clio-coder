...
| Supplied reasoning | One marker per run of it; a fold spans the reasoning between its calls | 3 rows before Clio's words, a marker before an action | 12 rows |
...
| Notices and provider retries | Level mark and one bounded message | Same | Same, with available diagnostic detail |
| Turn receipt | None | Outcome and available duration | Outcome, duration, usage, cache, model-call and speculative prewarm facts |
...
A running worker card states what it is doing now on its live line, and Detailed adds the call it finished last (`⚙ last: read docs/retry.md`). A settled card in Detailed lists the calls it made, oldest first and in the past tense (`⚙ read docs/retry.md`, `⚙ searched retry|options`), marks failed and blocked calls, and names earlier calls omitted by its four-action trail. Settled entries do not advertise stale current work. Pending checkpoint questions remain visible even in Compact.

Use **/view transcript** to select full available reasoning, tool arguments/results, local shell output, or worker details. Bare `/view` starts on the first displayed transcript detail, ahead of later global artifacts; clearing a filter keeps the selected item while exposing the other categories. Each entry is titled by what its transcript row states (`$ ran \`npm test\` · exit 1`, `± edited src/net/retry.js · +1 -1`, a notice's text, a worker card's header, reasoning by its opening line), its age is the time the act happened (the ledger's time on `/resume`), and a tool call's id stays searchable. The preview heads with up to three rows of the selected title and renders a transcript block at the pane's width, so wrapped rows keep their rail and hanging indent. Search the list, press Enter to inspect, use `n`/`p` to move between entries while inspecting, and Escape to return. Offloaded tool and dispatch output remains available in the other `/view` categories; missing or truncated captured content is identified. Inspection applies secret redaction to list titles and preview content, neutralizes terminal controls in supplied reasoning, notices and operator command echoes, and keeps `!!` output excluded from model context.

At narrow widths, the preview header keeps its position and a whole `i info` hint; the footer states item navigation and the way back. When a styled preview row contains a secret, inspection redacts its plain text before display so a partial ANSI color sequence cannot remain after the secret.
...
- `action` neon orange remains scarce and strictly disciplined: only while Clio is acting, for workspace-authority and worker-escalation decision frames, or during exceptional composer preparation/compacting phases. It is never used for idle decoration or settled telemetry, and never appears on more than one element per screen region. The transcript never uses `action`. It appears on the dispatch phase pill, the composer's preparing, compacting and steer rails, the steer queue marker, a boot-queued prompt, and the glyph of a running Fleet row. Queued Fleet rows and running status words stay muted. Outward, safety-net, and system decision frames use `warning`; conversational answers use `accent`.
...
- The 0.5.4 gallery audit rendered 15 scenes in all three output styles at 40, 60, 100, and 200 columns, both with color and with `NO_COLOR`. All 180 paired frames had the same plain text, no row exceeded its width, and no transcript frame used `action` orange. A live Herdr session confirmed the transcript, `/view`, and `/resume` remain distinguishable without color at 40 columns.
...
Quitting fullscreen restores the main screen without printing the docked header, composer and footer over it.
...
- **Compact Mode (Two-Line Ambient Strip)**:
  - **Line 1 (Workload & Identity)**: Active phase (`accent` e.g. `Ready`) and worker counts (`agent` e.g. `· 2 active`) · the armed skill surface while one narrows the tools (`skill tdd` in `muted`, or `§ tdd` where the words do not fit, dropping whole names as `§ tdd…`; it outranks everything after it) · Target/model identity fitted from its structured fields · Selected model’s weekly quota (`weekly N% left`, when the local credential owner and model group are known) · Thinking level (`reason`) on the left; Context meter bar (8–14 cells), its percent and token occupancy (`used / contextWindow`) on the right. Nothing on the row is cut mid-number or cut to a stub. The phase and worker count take the room they need beside the skill badge, and a row too narrow for both drops the worker count before it cuts the phase. The identity shows only with at least 12 cells of room; it drops placement and cuts at model-token boundaries to retain a complete suffix (`@q4_k_m`). The thinking level takes the room it leaves. A meter whose counts do not fit in about half the row drops them and keeps the percent (`▰▱▱▱▱▱▱▱  12.9%`). The phase spinner steps once per 120 ms animation step, however often the footer refreshes, and a live elapsed counter shows whole seconds (`Writing · 3s`) and nothing under one second.
  - **Line 2 (Status & Hints)**: Persistent workspace cwd and Git branch/dirty (`*`) on the left; rotating shortcut hints on the right. Urgent input prompts (`Ctrl+C again to quit`, `Ctrl+G → choose key`) keep their whole instruction at 40 columns by taking room from the workspace label. Active notices (`• text`, such as `• cache may be cold: prompt recompiled` before a response the context engine expects to miss the prompt cache) and demo tips borrow only the hint area. Quota never occupies this row.
...
| `✦` (accent) | Agent prose | Each prose block, not only the first of a turn, so narration that resumes after actions reads as the agent speaking again. A skill the model suggests is not prose: it reads `§ suggests /skill <name>` ahead of the answer. |
...
Supplied reasoning stays in stream order on the `reason` gutter rail, dim and italic, one block per run of it: stretches that nothing visible separates read as one. Compact shows the folded marker on the same rail, and a settled turn's last marker states how much reasoning the provider reported (`Thinking · /view · 241 tokens`, `≈` when Clio estimated it). A thinking model reasons before nearly every call, so a Compact fold spans that reasoning: the reasoning between its calls joins the one marker ahead of the fold instead of splitting it into one-call groups. Standard shows the bounded tail of the reasoning behind Clio's words (narration or the answer) and of reasoning still streaming, and the marker for reasoning behind an action; Detailed shows the bounded tail everywhere ([Output styles](#output-styles)), with the overflow hint on the rail. The footer alone reports current activity. The excerpt is what the provider supplied, not verification.
...
| --- | --- | --- | --- | --- |
| observe | read, ls, data, credential_present | `▸` | reading / read | path, `lines a-b of N` or a count of entries, size (not for a listing), resource kind |
...
A command already runs at the workspace root, so a leading `cd` into the workspace is dropped from its row (`$ ran \`npm test 2>&1\``, not a row that shows the path and cuts the command), and a `cd` into a subdirectory or an explicit `cwd` argument reads as its scope (`$ ran \`ls\` in src`); a `cd` anywhere else stays in the command. A timeout the command stayed under is not stated, and one it hit is the fact (`timed out after 30s`). An offloaded result ends its row with `· full output · /view`; the scratch path is in `/view` and in the full body's footer. A row cuts its object with `…`. A failed command states its exit status once, as the `exit N` fact, and its body drops the status line (`bash: command failed (exit 1)`, `Command exited with code 1`, a timeout or an abort) unless that line carries the only diagnosis. A row states an exit code only when the result carries one. A blocked call's verb is `blocked` and its tail names the refusal. A failed call always shows its bounded body, so the row never excerpts it. A successful edit or write carries its change facts (`+5 -2`) on the row in every style and never previews its replacement payload; a failed one keeps the payload, bounded, because the text that did not match is the diagnosis. A question to the operator reads `question → answer`; a round of several questions, or an interview's closing decisions, nests one `question → answer` row each, and the model's copy of the interview never renders. The row is the round's record, so the overlay adds none; only a question Clio asks outside any tool call, such as whether to install a skill `/skill` named, leaves a `❯ question` / answer record. A dispatch row states its execution tally (`3 ok`, `4 ok, 1 failed`) and, when no worker card sits under it, its receipts' validation quality; a card under the call states the run's task and outcome instead ([6.9](#69-agent-invocations)). A row too long for the terminal wraps with a hanging indent into the content column.

A skill load is its own action identity. A settled `context(scope="skills")` load reads `§ loaded skill <name>` with the name in `accent`, states who asked for it (`by operator` for `/skill`, the selector or a marketplace install; `by recipe` for a bound worker; `by model` under model activation), and `drifted` in `warning` when its content no longer matches its recorded hash. Standard and Detailed nest the tool surface the skill declares (`narrows tools to read, edit, bash`, or `narrows tools to all but bash` for an exclusion list) and its description, one row each. A refused load reads as plainly as a load: what did not load, why, and the one move that changes it, on one row with no body (`§ skill tech-spec not loaded · manual-only: /skill tech-spec ✗`). The row reads the structured refusal the context tool returns in its error details, never the message, which is the model's instruction and stays in `/view`:

| Refusal | Row states |
| --- | --- |
| `manual-only` | `manual-only: /skill <name>` |
| `operator-only` | `only you can load it: /skill <name>` |
| `not-installed` | `not installed: /skill <name>` |
| `not-ready` | `installed but <state>: /library` |
| `untrusted` | `untrusted: review it in /library` |
| `not-imported` | `not imported: found in <source>/<scope>` |
| `recipe-bound` | `not declared for this run` |
| `not-requested` | `not requested this turn` |
| `already-loaded` | `already loaded` |
| `unknown` | `unknown skill` |

The main agent returns such an error instead of throwing it, because a thrown error reaches the transcript with its details replaced by `{}`. An operator prompt that starts with `/skill <name>` leads with that command in `accent`, live and on `/resume`: the typed line is the prompt's persisted display text. When the model suggests a skill instead of loading it, its `Suggested skill: /skill <name>` line becomes `§ suggests /skill <name>` ahead of the answer, with the command in `accent`.
...
Paths inside the workspace read relative to it. A path that fits beside its verb stays whole; a path that must be cut keeps its tail (`…/net/retry.js`) on the action row. The resource label (`· docs`, `· skill`) appears only when the path has not already named that resource. Short facts such as `18 lines` and `context 3.2k` stay together when a row wraps. A blocked row states its refusal once: the bounded body omits the opening line already quoted on the row, and a safety-net block uses the footer notice slot. The safety layer marks a call target that exceeds 120 characters with `…`; an exactly 120-character target stays unmarked. A monitor preview drops a leading run id already named by its action row, while inspection keeps the complete result.
...
✓ [/export] wrote 2301 lines to
  .clio-coder/exports/1t6ygqmc7kuu-2026-09-23.html
```
...
An export inside the workspace names its path relative to that workspace in the success notice. A destination outside it keeps its absolute path.
...
The chat loop emits the speculative counts after the turn's session entry is written. The panel adds them to its settled receipt, and `/resume` reads the entry for the same fact.
...
The settled trail persists in `workerSettled.calls`, including whether each call succeeded, failed or was blocked, so `/resume` shows the same `✗ blocked` and `✗ failed` marks. Repeated identical calls compress to one `×N` row, and a descriptor cut by the safety layer ends in `…`. When a receipt counts more calls than the bounded trail contains, the trail begins `⚙ … N earlier calls · /view dispatch:<runId>`. The inspected card in `/view` and `/export` lifts the transcript's row budgets, keeps the full available raw answer, and shows contract conformance beside spend. If a card arrives after its dispatch settles, it still nests beneath the call that spawned it. A pending live answer tail strips dead tool-call markup; inspection and export retain the worker's available raw answer.
...
Helper and shadow work is one subordinate row outside Detailed: the helper, what it was asked to do (or, while it runs, the call it is making), and how it ended (`↳ context-scout · Summarize prior probe benchmarks ✓ · 7.3s`). A failure adds its reason on one nested row. Detailed shows the full card. Consecutive helper rows stack, and every run of cards stacks in Compact. A shadow helper the model started with `dispatch` (its `scout`) is that call's row like any other card: the call reads `◆ delegated to scout ✓ · 11s` and the `↳` row states the task once. Under a call that is not a dispatch, a helper sits beside that call's own output. A call with a card under it states no arguments at all, its route included, because the card owns the run.
...
Workspace outputs lead with their basename and workspace-relative path. The preview heads with the selected title, then its category, time and available size, instead of leading with an internal artifact id. In preview, `n`/`p` select the next or previous result without returning to the list, `i` toggles scrollable provenance including the full backing path and available session, run and correlation identities, `o` keeps its backing-path notice and `v` retains available verification. Toggling provenance resets that pane's scroll offset. Below 84 columns the list and preview each use the whole width, with Enter opening preview and Esc returning to the list before closing; at wider widths the list grows to 72 columns so action titles remain readable. Empty results cannot acquire preview focus. Content loading and width-specific layout are deferred off the synchronous render stack, and stale loads or layouts cannot replace newer selection/results.
