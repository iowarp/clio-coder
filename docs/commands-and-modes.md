...
> [!TIP]
> **Interactive Spec Available:** An interactive dashboard is located at [docs/html/commands_blueprint.html](html/commands_blueprint.html) (Version: 0.3.5).
...
When an action asks for confirmation, whether from a safety-net rail or from the autonomy level, the call parks and three surfaces say so at once. The transcript row reads `⏸ awaiting approval` with `action ·`, `axis ·`, and `target ·` lines under it; the footer phase pill reads `⏸ confirm`; and a dialog titled `Allow this action once?` opens with `Tool:`, `Target:`, `Action:`, and `Asked by:` lines, where `Asked by:` names the asking axis (`safety-net rail <rule>` or `autonomy level (<level>)`). The dialog sits at the vertical center of the viewport, so on a tall terminal with a long transcript it can be far from the composer; the composer rail therefore switches to `CONFIRM` and repeats the keys while the prompt owns the keyboard.
