...
<dataDir>/evidence/<evidenceId>/
  ...
  audit-linked.jsonl
  receipt.json
  eval-result.json         # only for eval sources
  findings.json
  findings.md
  protected-artifacts.json   # only when protection events were recorded
```

Inputs are read from the standard XDG layout: `<dataDir>/receipts/<runId>.json`, `<dataDir>/state/runs.json`, `<dataDir>/sessions/<sessionId>.jsonl`, and `<dataDir>/audit/YYYY-MM-DD.jsonl`. Eval-sourced corpora additionally read the persisted artifact at `<dataDir>/evals/<evalId>.json`. The builder strips or truncates very large outputs and preserves command context (phase/tool command), exit code, timed-out status, duration, blocked counters, and validation hints.
...
- `clio evidence build --eval <evalId>` rebuilds a corpus from a persisted eval artifact. It uses the same `buildEvalEvidence` path as `clio eval run`.
...
- `tests/integration/evidence-builder.test.ts`, `tests/integration/eval-evidence.test.ts`: regression coverage.
