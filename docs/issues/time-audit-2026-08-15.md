...
| 12 | **Medium** | Audit files are named by **local** date while their rows carry **UTC** instants. The only local time anywhere on disk | (b) fragile | `src/domains/safety/audit.ts:190-198,456` | Accepted (documented as a human-facing label, `a548e8c8`) |
...
| 16 | **Low** | The worker heartbeat frame carries `at` from the *worker's* clock. Parsed, required, never read | (b) fragile | `src/worker/heartbeat.ts:18` + `src/worker/protocol.ts:485-488` | Fixed (Phase 3, `21d01846`) |
| 17 | **Low** | Context fingerprint is `size` + floored `mtimeMs`; blind to same-size edits under coarse or cached filesystem attribute granularity | (b) fragile | `src/domains/context/fingerprint.ts:118` | Open |
