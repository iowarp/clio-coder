...
> [!TIP]
> **Interactive Spec Available:** An interactive dashboard is located at [docs/html/tools_blueprint.html](html/tools_blueprint.html) (Version: 0.2.8).
...
- `dispatch(tasks, mode?, ...)` takes an array of task strings or `{agent, task, ...}` objects and runs them `parallel` (default), `sequential`, or `pipeline`. Ordinary handles enter one registered event consumer immediately. Synchronous calls auto-wait for that consumer's stream-and-receipt completion; `detach:true` returns ids after durable batch registration while the same consumer continues, which is how the parent model obtains mid-run monitor/steer control. Review and compete are gate-sensitive direct-drain exceptions so reviewer/judge output is staged before their receipt-facing await. A top-level `briefing` supplies a default and a task-object `briefing` overrides it; blank values are omitted and non-empty values above 12,000 UTF-8 bytes are rejected. Approval artifacts pin its exact canonical value and show bounded preview/byte/hash facts. Receipts and ledger rows retain only `briefing.bytes` and `briefing.contentHash`, not prose. Task objects may also include `persona` to compose an ad-hoc specialist by replacing the recipe body inside the stable worker shell, plus `tool_profile` (`minimal-local`, `science-local`, or `full-agent`) to narrow that worker's tools. Personas are capped at 8000 characters, rejected for shadow and ACP delegation agents, and receipts record `personaOverride.promptHash` equal to the composed run's `staticCompositionHash`. In `pipeline` mode tasks run one at a time and each step's final assistant output is threaded to the next step as a delimited dynamic prompt message labeled as data, not instructions; step 1 receives none. The threaded text is capped at 12000 characters, and the receiving run's receipt records `pipeline` provenance (`fromRunId`, `position`, `inputBytes`, `inputTruncated`). A failed step halts the chain and later steps are reported as skipped. The output is one per-run receipt summary per task, even for a single task. `dispatch(list=true)` renders the agent catalog.
- `monitor(run_id?, mode?)` is read-only visibility into known synchronous and detached dispatched runs: `mode=list` enumerates known runs, `status` (default with a run id) reports state and progress counters, `peek` returns the rolling tail of recent events buffered in this process, `receipt` returns the stored receipt JSON with a 14KB cap, `wait` observes one run to a bounded timeout, and `collect` gathers a detached batch or explicit run-id list. The interactive operator/TUI can inspect an active synchronous run through the dispatch contract; the parent model must choose detached dispatch because a sequential synchronous dispatch call blocks its next tool call.
- `steer(run_id, action, message?)` controls a running worker: `action=guide` injects a steering message the worker sees as a user message at its next turn boundary (native workers only; ACP surfaces the dispatch contract's structured no-input-channel error), and `action=cancel` terminates the run cleanly, with the receipt recording the cancellation. The interactive operator/TUI can steer active synchronous native runs; parent-model mid-run steering requires ids returned by detached dispatch because dispatch and steer are sequential tools.
