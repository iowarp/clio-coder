...
> [!TIP]
> **Interactive Spec Available:** An interactive dashboard is located at [docs/html/tools_blueprint.html](html/tools_blueprint.html) (Version: 0.2.9).
...
- `steer(run_id, action, message?)` controls a running worker: `guide` writes a canonical trimmed steering message to a native worker and `cancel` terminates it. Successfully written steers gain ordered byte/hash/timestamp provenance; `clio_steer_received` marks the oldest unacknowledged entry, and prose is never stored in ledger or receipt. ACP remains non-steerable. Interactive operators can steer synchronous native runs; parent-model steering requires detached ids because model tools are sequential.
