...
A running session does not read installed packages on every resource load. At boot the extensions domain builds one immutable snapshot of every installed package for the session's working directory: package identity and provenance, the resolved skill, prompt, agent, fleet, and theme roots of each loadable package, and the parsed `hooks.yaml` declarations captured from the exact bytes the install digest covered. The snapshot is generation 1. Every consumer in the process reads that committed generation, so consecutive loads within one turn always agree on the package set.

`/resources extensions reload` is the only in-session way to publish a new generation. It rebuilds the snapshot from disk, re-verifies every installed tree against `state.json`, builds the user-hook registrations for the candidate, and only when both are valid commits the snapshot and the hook registrations together. Observers see the previous resources with the previous hooks or the new ones with the new ones; there is no intermediate state. The command reports the new generation, which packages were added, removed, or modified, and how many hooks were registered, dropped, or rejected. A tree that no longer verifies is listed as inactive and contributes nothing until it is reinstalled. A build failure or a stale candidate leaves the active generation in place and reports why.

Reloading an unchanged tree still commits a new generation with the same content digest; content identity is the digest, not the generation number. Installs, enables, disables, and removes performed by `clio-coder extensions` in another process are invisible to a running session until the operator reloads or restarts. There is no filesystem watcher, so a CLI mutation never becomes an implicit mid-turn hook change. Resource files themselves (skill and prompt bodies, agent and fleet recipes) are still read at use time; a package mutated on disk after its generation was built can serve changed files until the next reload detects the drift and deactivates it.
