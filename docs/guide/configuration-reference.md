...
| `fleet.concurrency` | `1` | Worker limit for the global pool and the local node, or `auto` to size the local node from usable CPUs, available memory, and any cgroup memory limit (at most 8; see [Fleet dispatch](fleet-dispatch.md#worker-limits-and-fleetconcurrency-auto)); an integer >= 1 is exact; applies at restart. |  |
