...
| `fleet.decisionProfiles` | `{}` | Map of harness decision site to a `fleet.profiles` name naming the System One model that answers it. Sites are `routing` (retired: accepted, never asked, and named in one startup notice), `skills` (which installed skills the listing carries), `memory` (which durable records the prompt carries), `toolRisk` (advisory blast-radius rating at the approval prompt), `drafts` (which `/draft` candidate is strongest), `turnScope` (a pre-turn hint that the request needs nothing from the workspace), `dispatchForecast` (a pre-turn hint that the request suits workers, and in what shape), `capabilities` (which gateway capabilities fit what a find asked for), and `consult` (typed questions the main agent asks through the gateway). A site with no entry is off and its caller keeps the behavior it had before the site existed; applies next turn. | [Session override → saved settings](#reading-the-precedence-column) |
| `fleet.decisionProfiles.routing` |  | Retired. A bound value still validates, is never asked, and produces one startup notice. Dispatch admission never awaits a decision model and always classifies a task with its rules. |  |
...
| `fleet.speculativeDispatch` | `false` | Experimental. With `dispatchForecast` bound, a confident forecast of the recipe the main agent will dispatch first starts that worker's process ahead of the call and holds it, at most two, until a dispatch matching its recipe, target, model, runtime and directory adopts it or the turn settles. The agent's dispatch is never changed or delayed. Off, the forecast asks nothing new and nothing is held; applies next turn. |  |
| `fleet.decisionProfiles.consult` |  | `fleet.profiles` name whose System One model answers the typed questions the main agent sends through `gateway` call `consult`: up to 3 calls a turn, 4 questions a call and 2 KB of evidence. The answer is a distribution returned as advice and never picks. Unset, the capability is not registered and the gateway listing and prompt are unchanged; applies next session. |  |
