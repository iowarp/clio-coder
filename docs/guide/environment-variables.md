...
The `environment-variable-inventory` check in [check-hygiene.ts](../../scripts/check-hygiene.ts), run by `pnpm run lint`, enforces coverage for Clio's `CLIO_*` variables and `NO_COLOR`. It intentionally does not treat every operating-system or provider convention as a Clio knob. Examples outside that enforced family include `PATH`, `HOME`, terminal capability variables, and provider API-key names selected dynamically by [env-api-keys.ts](../../src/engine/env-api-keys.ts).
...
| `CLIO_CODER_ALLOW_EXTERNAL_FULL_ACCESS` | off | `1` lets full-auto pass through to external CLI runtimes with their own full access ([subprocess-runtime.ts](../../src/engine/claude/subprocess-runtime.ts), [subprocess-runtime.ts](../../src/engine/antigravity/subprocess-runtime.ts)). |
