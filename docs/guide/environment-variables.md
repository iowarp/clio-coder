...
| `SLURM_JOB_ID`, `SLURM_JOB_NAME`, `SLURM_CLUSTER_NAME`, `PBS_JOBID`, `PBS_JOBNAME`, `PBS_O_HOST`, `LSB_JOBID`, `LSB_JOBNAME`, `LSF_CLUSTER_NAME` | Scheduler allocation identity stamped into run receipts. Slurm is checked first, then PBS, then LSF, and the first job id found wins; its job name and cluster (`PBS_O_HOST` for PBS) are recorded with it. Outside an allocation the receipt's `hpc` field is null ([run-identity.ts](../../src/domains/dispatch/run-identity.ts)). |
