...
> [!TIP]
> **Interactive Spec Available:** An interactive configuration validator, target resolver, and CLI command generator is located at [docs/html/configuration_blueprint.html](html/configuration_blueprint.html) (Version: 0.3.5).
...
When the selected instance is also loaded on a peer, a request may be answered by that peer, so Clio attributes each turn to the model the server reported rather than the one it asked for (#185). The adapter records a differing response `model` field as `responseModel` on the assistant entry. It also records the exact id from an observed OpenAI-compatible event stream as `servedModel`. A `null` value means the observed stream contained no model id, and receipts and usage render that fact as `unknown`. Entries from providers the stream tap does not cover keep the historical `responseModel` fallback. The footer's last-turn line then adds `served <id>`, the `/cost` overlay and `clio-coder usage report` attribute the tokens to the served id and list the requested ids beside it (`same` when the ids match), and dispatch receipts carry both ids on `upstreamResponses`. The peer warning is said once per process per distinct fact (target, requested id, resolved instance, peer set), not once per turn.
