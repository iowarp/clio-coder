```
You are executing Phase 11 of the Clio-Coder
  pi-coding-agent port.

  Repo: /home/akougkas/projects/iowarp/clio-coder
  (branch: main)

  Read these three files in order, then execute:
  1. docs/specs/2026-04-16-clio-coder-design.md
         (architecture + invariants)
  2. docs/superpowers/plans/2026-04-17-pi-coding-agent-p
  ort.md (the spine; §2 is the
     IP-to-destination map, §4 Phase 11 is the scope)
  3. docs/superpowers/plans/2026-04-17-clio-phase-11-sel
  ectors.md (the executable plan)

  Invariants you must not break:
  - src/engine/ is the sole pi-mono import boundary.
  Every new TUI primitive goes
    through src/engine/tui.ts.
  - src/worker/ never imports from src/domains/.
  - No new npm deps in Phase 11. Use only what is
  already in package.json.
  - No tests (Clio's locked "no vanity test suite").
  Verification is scripts/diag-*.ts
    plus manual TUI drill for anything that needs a TTY.

  Execution mode: invoke the superpowers:executing-plans
   skill and run Tasks 1→13
  sequentially, committing at each task's final step as
  the plan dictates. After
  Task 13, report CI status (npm run ci) and leave the
  tree clean for review.

  Do not expand scope. Items the plan defers to Phase 12
   (message-picker,
  session-selector-search, /tree) stay deferred. If a
  step turns out wrong, fix
  it inline and keep going; do not rewrite the plan.
```
=========
