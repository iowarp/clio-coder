...
|---|---|---|---|
| `AgentToolResult.terminate?: boolean` | `agent/types.ts` | `src/tools/write-plan.ts`, `src/tools/write-review.ts` | ✅ Landed. `ToolResult` gained an optional `terminate` on the `ok` variant; advise-only writers set it on success, and `src/engine/worker-tools.ts:toAgentTool` forwards it onto `AgentToolResult.terminate`. Skips the follow-up LLM call when the tool batch is fully terminal. Advise-only admission removes the need for an `afterToolCall` override. |
| `Terminal.setProgress(active)` | `tui/terminal.ts` | `src/engine/tui.ts` (`createAgentProgress`) + `src/interactive/index.ts` (agent_start/agent_end subscriber) | ✅ Landed. Engine helper wraps OSC 9;4 with idempotent start/stop so terminals like WezTerm, Ghostty, Konsole, and Windows Terminal show taskbar/tab progress while the agent is working. Chat-loop emits `agent_start`/`agent_end`; the interactive subscriber toggles the helper. |
| `AgentTool.executionMode` | `agent/types.ts` | `src/tools/registry.ts` (`ToolSpec.executionMode`) + `src/engine/worker-tools.ts` | ✅ Landed. Read-only tools (`read`, `grep`, `glob`, `ls`, `web_fetch`) carry `"parallel"`; mutating/bash tools (`write`, `edit`, `bash`, `write_plan`, `write_review`) carry `"sequential"` so concurrent `bash` or `edit` calls in the same batch never race. |
...
| `AgentTool.executionMode` per-tool override | re-exported | ✅ Wired. `src/tools/registry.ts:ToolSpec.executionMode` forwarded by `src/engine/worker-tools.ts:toAgentTool`. Read-only tools (`read`, `grep`, `glob`, `ls`, `web_fetch`) → `parallel`; mutating/bash tools (`write`, `edit`, `bash`, `write_plan`, `write_review`) → `sequential`. | n/a |
...
| `Terminal.setProgress(active)` (new in 0.69.0) | not re-exported directly; wrapped via `src/engine/tui.ts:createAgentProgress` | ✅ Wired at `agent_start`/`agent_end` in `src/interactive/index.ts`. | n/a |
...
| `--api-key <key>` startup flag | `src/cli/index.ts` + `src/entry/orchestrator.ts` + `src/cli/run.ts` | ✅ | Pre-parsed via `extractApiKeyFlag()` in `src/cli/shared.ts`, threaded into `bootOrchestrator(options)` and `runClioRun(args, options)`. After providers domain loads, `providers.auth.setRuntimeOverrideForTarget()` installs an in-process key keyed by `endpoint.id` (not providerId) so the flag only affects the active endpoint and does not leak to sibling endpoints that share the same runtime. `AuthStorage.runtimeOverrides` short-circuits `resolveApiKey()` when the caller passes `endpointId`, so chat-loop and dispatch both honor the override. Persists nothing. `clio --help` documents the flag; orphan use without an active endpoint warns and exits 0. |
