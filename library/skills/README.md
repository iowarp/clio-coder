...
is not a claim that a model evaluation passed. Authors can start by copying the
standalone skill template from `library/_authoring/templates/skill/` (with `eval-status: scenarios-recorded`
prior to maintainer review). The existing `skill-craft` skill provides the authoring and review procedure.
