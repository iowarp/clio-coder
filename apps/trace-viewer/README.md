...
rowid-cursor polling contract from `docs/trace-store.md`. The browser polls a
running run every 500 ms; finished history uses the same API and reducer.

The viewer reads only `trace.sqlite`. It never writes or archives a run, opens
receipts or ledgers, signals a process, contacts an external service, or binds
to a remote interface. `apps/` is intentionally absent from the published npm
artifact, so the launcher is a source-checkout development surface.
