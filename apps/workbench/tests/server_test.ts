...
Deno.test("toolchain inspection is global, path-free, and survives browser reload", async () => {
	let inspectedCwd: string | null = null;
	const toolchainInspector: ClioToolchainInspector = {
		inspect(cwd) {
			inspectedCwd = cwd;
			return Promise.resolve(toolchainInspectionFixture());
		},
	};
	const fixture = await startFixture({ toolchainInspector });
	let socket: RawWebSocket | undefined;
	try {
		socket = await RawWebSocket.connect(eventsEndpoint(fixture.running), fixture.running.url);
		equal((await socket.readEvent()).kind, "connection.ready");
		await sendCommand(socket, "request-toolchain", "toolchain.inspect", {});
		const toolchain = (await collectThrough(socket, "toolchain.state")).at(-1);
		ok(toolchain?.kind === "toolchain.state");
		equal(toolchain.projectId, undefined);
		equal(inspectedCwd, fixture.homePath);
		deepStrictEqual(toolchain.payload.inspection, toolchainInspectionFixture());

		const bootstrap = await (await fetch(new URL("/api/bootstrap", fixture.running.url))).json() as {
			toolchainInspection: WireToolchainInspection;
		};
		deepStrictEqual(bootstrap.toolchainInspection, toolchainInspectionFixture());
	} finally {
		await socket?.closeGracefully().catch(() => socket?.closeAbruptly());
		await fixture.close();
	}
...
Deno.test("external agent detection is global, uses the configured home, and survives browser reload", async () => {
	let inspectedCwd: string | null = null;
	const interopInspector: ClioInteropInspector = {
		inspect(cwd) {
			inspectedCwd = cwd;
			return Promise.resolve(interopInspectionFixture());
		},
	};
	const fixture = await startFixture({ interopInspector });
	let socket: RawWebSocket | undefined;
	try {
		socket = await RawWebSocket.connect(eventsEndpoint(fixture.running), fixture.running.url);
		equal((await socket.readEvent()).kind, "connection.ready");
		// What is installed on this machine is not a fact about any one project.
		await sendCommand(socket, "request-interop", "interop.inspect", {});
		const interop = (await collectThrough(socket, "interop.state")).at(-1);
		ok(interop?.kind === "interop.state");
		equal(interop.projectId, undefined);
		equal(inspectedCwd, fixture.homePath);
		deepStrictEqual(interop.payload.inspection, interopInspectionFixture());
...
Deno.test("evidence inspection is global, uses the configured home, and survives browser reload", async () => {
	let inspectedCwd: string | null = null;
	...
	try {
		socket = await RawWebSocket.connect(eventsEndpoint(fixture.running), fixture.running.url);
		equal((await socket.readEvent()).kind, "connection.ready");
		await sendCommand(socket, "request-evidence", "evidence.inspect", {});
		const evidence = (await collectThrough(socket, "evidence.state")).at(-1);
		ok(evidence?.kind === "evidence.state");
		equal(evidence.projectId, undefined);
		equal(inspectedCwd, fixture.homePath);
		deepStrictEqual(evidence.payload.inspection, evidenceInspectionFixture());
