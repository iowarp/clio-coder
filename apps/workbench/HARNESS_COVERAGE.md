...
current Workbench host/renderer protocol. It made no changes under the repository's root `src/`. The historical-record
...
| Agents and recipes                         | Searchable read-only cards for recipe, provenance, capability, latency, context tier, bindings, result contract, and budget                              | **Wired** for discovery; **Upstream boundary** for execution                           | A fixed `clio-coder agents --json` adapter projects a bounded inventory and drops native file paths. Addressable execution still requires dispatch operations/events.                                                                               |
...
| Evidence artifacts                         | Usage records name the typed-listing gap; no evidence rows are fabricated                                                                                | **Upstream boundary** for typed discovery                                              | Public build/inspect/list commands exist, but `evidence list` is formatted text rather than a stable machine schema. Workbench does not scrape it.                                                                                                  |
| Evaluations                                | Usage records name the missing eval-discovery bridge; no suite or comparison surface                                                                     | **Workbench-ready** for a known eval; **Upstream boundary** for discovery              | JSON reports exist after an eval ID is supplied, but no typed listing lets Workbench safely discover which project eval to open. Add that listing before building the Experiment index.                                                             |
...
| Skills                                     | Searchable installed inventory with scope, source, trust, precedence, invocation posture, and issue counts                                               | **Wired** for inventory; **Workbench-ready** for deeper read-only flows                | A fixed `skills list --json` adapter omits bodies, hashes, native locations, and diagnostic text. Inspect/validate/eval remain candidates; install/update/sync need review and confirmation, and live activation needs upstream events.             |
...
| Library                                    | Searchable available-resource inventory with kind, category, version, origin, and audit state                                                            | **Wired** for inventory; **Workbench-ready** for deeper read-only flows                | A fixed `library list --json` adapter omits source URLs and requirements. Search can remain local over the bounded projection; use/sync/push/add need a separately reviewed resource flow.                                                          |
| Project verifiers                          | Capability atlas names the missing typed interface; no verifier rows are fabricated                                                                      | **Upstream boundary** for typed discovery                                              | `verifiers discover` is formatted authoring output, not JSON. Workbench refuses to scrape it. Add a typed listing before checks become cards; author/validate/dry-run/mutations must preserve exact argv/cwd/timeout/tags/authority.                |
...
3. **Capability atlas — partially implemented in this slice.** Fixed read-only adapters now project Agents, installed
   Skills, and Library resources into independently fallible, bounded collections with search, provenance, trust, and
   budget facts. The Verifiers tab deliberately remains an interface-boundary explanation until Clio publishes a typed
   listing; Workbench does not scrape its formatted authoring preview.
...
discarded, and the three catalog reads execute in parallel while failing independently. Host integration tests cover
that isolation. Future command adapters should retain the same fixed-argv, bounded-output, typed-projection discipline.
