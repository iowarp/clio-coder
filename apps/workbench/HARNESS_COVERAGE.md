...
Workbench protocol v3 currently validates 24 client commands:
...
`settings.patch`, `targets.list`, `targets.probe`, and `autonomy.set`.

It validates 22 server event kinds:
...
`turn.started`, `turn.text`, `turn.thought`, `turn.tool`, `turn.loop`, `turn.permission.requested`,
...
| Config provenance                          | Only four safe settings are shown                                                                 | **Workbench-ready** for read-only inspection                                           | `clio-coder config inspect --json` is the strongest next scientist-facing surface: effective settings, context, rules, skills, prompts, agents, extensions, safety, memory, hooks, operator profile, provenance, trust, reload class, and context cost. |
...
2. **Effective Clio map.** Adapt `config inspect --json` read-only in the Workbench host and visualize loaded settings,
   context, prompts, rules, agents, skills, extensions, hooks, trust, precedence, reload timing, and context cost. This
   is unusually valuable to non-engineers because it answers “why is Clio behaving this way?” without asking them to
   read YAML or inspect directories.
...
interfaces. Whether a hosted Workbench process may launch those read-only commands alongside its ACP child should be
made an explicit product architecture decision and covered by concurrency/state-lock tests.
