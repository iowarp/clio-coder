...
- Bounded host adapters cover fixed `config inspect --json`, `agents --json`, `skills list --json`, and
  `library list --json` reads without exposing their raw command output to the renderer.
...
Workbench protocol v3 currently validates 26 client commands:
...
`settings.patch`, `targets.list`, `targets.probe`, `autonomy.set`, `config.inspect`, and `catalog.inspect`.

It validates 24 server event kinds:
...
`config.state`, `catalog.state`, `turn.started`, `turn.text`, `turn.thought`, `turn.tool`, `turn.loop`,
...
interfaces. Config and catalog inspection now run through a separate serialized request lane alongside ACP: slow reads
cannot block turn cancellation or permission resolution, late results from a previously selected project are discarded,
and the three catalog reads execute in parallel while failing independently. Host integration tests cover that
