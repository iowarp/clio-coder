...
- **TUI polish and usage clarity.** Tool output, thinking blocks, popups, `/cost`, `/model`, `/hotkeys`, `Ctrl+G` external editing, `!cmd` / `!!cmd` local bash, and receipt views have been hardened for long text, terminal width, local usage accounting, and clearer visual hierarchy.
