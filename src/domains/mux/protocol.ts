 ...
 * protocol range Clio cares about. Phase 3 adds two that do not: a herdr old
 ...
 *
 * The floors are the lowest protocol whose `herdr api schema --json` was
 * actually read, not a guess. Both methods are present in protocol 17
 * (herdr 0.7.5) and in the pinned 0.8.2, so 17 is the floor for both. Lower one
 * only after checking the schema of the release you are lowering it to.
 */

import type { MuxServerInfo } from "./types.js";

/** Wire methods this file gates. Phase 1 methods are unconditional and absent here. */
export type MuxGatedMethod = "notification.show" | "agent.focus";
