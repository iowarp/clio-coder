...
Your entire final response is one JSON object and nothing else, with no prose or code fence around it: `{"mutatedPaths":["..."],"validations":[{"name":"...","passed":true,"evidence":"..."}]}`. `mutatedPaths` carries workspace file paths this run actually changed and nothing else; the orchestrator checks each one against what your tool calls did, so a path you did not touch fails the run. Report branch, ref, and commit changes as validations, with the command output as evidence. For pure-inspection tasks report an empty mutatedPaths and the inspection evidence as a validation.
