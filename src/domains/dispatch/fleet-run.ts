...
import { type ExecutionPlanResult, executePlan } from "./execution-scheduler.js";
...
export async function executeFleetRun(input: ExecuteFleetRunInput): Promise<FleetRunOutcome> {
	...
	const notice = (text: string): void => input.onNotice?.(text);
	...
	const boundaryEnforcer = createWriteBoundaryEnforcer({
		...
		onVerdict(verdict, path) {
			...
			notice(`write boundary ${verdict.window}: ${verdict.detail ?? verdict.reason} record=${path}`);
		},
	});
