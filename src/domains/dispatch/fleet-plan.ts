...
export function compileFleetExecutionPlan(input: CompileFleetPlanInput): ExecutionPlan {
	...
	for (const step of contract.steps) {
		...
		if (step.kind === "agent") {
			steps.push(agentNode({ stepId: step.id, agentId: step.agent, scope: step.scope, attempt: 0 }, dependencies));
			continue;
		}
		if (step.kind === "code") {
			steps.push({
		...
		for (let attempt = 1; attempt <= step.maxAttempts; attempt++) {
			...
			steps.push(
				agentNode({ stepId: repairId, agentId: step.repair.agent, scope: step.repair.scope, attempt }, [checkId], {
					loopId: loop.id,
					role: "repair",
					attempt,
				}),
