 ...
 * assignment registry do not survive a restart (Symphony §14.3), so every
 * durable record still marked `running` after a crash is orphaned: no active
 * attempt, no queued retry, and no attached terminal promise can ever settle it.
 *
