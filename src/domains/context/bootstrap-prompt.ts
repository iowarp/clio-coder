...
WHERE THE RULES ARE. The input's enforcement inventory lists the commands CI runs, the package scripts they reach, and the repository's custom check files with the check functions they define. Read every check file it lists and the configs those commands load. Each check that an ordinary change can fail is a rule the agent breaks without noticing, so write one rule for it: a change recipe when the check demands that files change together ("adding X requires Y and Z"), an invariant when it forbids something. Skip checks that a formatter or linter reports together with its fix. Then read the test harness setup, contributor guides and the sibling agent files you were given. A contributor guide the agent can read itself earns a line only when it states a rule no check enforces.
