...
import { isTargetEligibleRuntime } from "../domains/providers/index.js";
...
function hasUsableDefaultTarget(): boolean {
	...
	if (!isTargetEligibleRuntime(runtime)) return false;
...
export async function runClioCommand(options: BootOptions = {}): Promise<number> {
	...
	if (!options.headless && process.env.CLIO_INTERACTIVE === undefined && process.stdin.isTTY) {
